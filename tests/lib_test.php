...
require_once($CFG->dirroot . '/enrol/autoenrol/tests/testable_enrol_autoenrol_plugin.php');
...
final class lib_test extends \advanced_testcase {
     ...
     *
     * @covers ::enrol_autoenrol->email_welcome_message
     * [CoversFunction(::enrol_autoenrol->email_welcome_message)]
     */
    ...
    public function test_welcome_call_sites_route_through_shared_method(): void {
        ...
        $this->assertSame(0, preg_match_all(
            '/email_to_user\(\$user,\s*\$[A-Z_a-z]+,\s*\$subject,\s*\$messagetext,\s*\$messagehtml\)/',
            $contents
        ));
    }
    ...
    private function create_welcome_fixture(int $sendoption): array {
        ...
        $welcomehtml = '<p>Hello {$a->fullname}</p><div dir="rtl">ﺥﻮﺷ ﺂﻣﺪﯾﺩ</div><p><a href="{$a->link}">{$a->coursename}</a></p>';
